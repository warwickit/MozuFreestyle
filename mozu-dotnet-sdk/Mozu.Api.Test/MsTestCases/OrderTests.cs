...
namespace Mozu.Api.Test.MsTestCases
{
    ...
    public class OrderTests : MozuApiTestBase
    {
            ...
            bool returnOrder = false, bool anonymous = false, bool pickupMethod = true, bool directShipMethod = true)
        {
            ...
            long loopQty = 0;
            const int maxProductLineItems = 2; // max product line items 5
            const int maxProductItemsPerLine = 2; // max product line items 5
            loopQty = productUsageCollection.TotalCount < maxProductLineItems ? productUsageCollection.TotalCount : maxProductLineItems;
            ...
            for (var itemsLoop = 0; itemsLoop <= loopQty; itemsLoop++)
            {
                ...
                {
                    Debug.WriteLine("Product: " + productToUse + " is visible in ProductAdmin and not Storefront. Finding new product");
                    productToUse = productUsageCollection.Items[random.Next(conv)].ProductCode;
                    while (productsUsed.Contains(productToUse))
                    {
                        productToUse = productUsageCollection.Items[random.Next(conv)].ProductCode;
                ...
                Assert.AreNotEqual(product.IsActive, 0,
                                    "Product: " + productToUse + " is visible in ProductAdmin and not Storefront.");
                ...
                if (pickupMethod && PickupLocationCodes.TotalCount != 0 )
                {
                    ...
                    {
                        Debug.WriteLine("Product " + product.ProductCode + " does not need inventory to purchase.");
                    }
                    else 
                    {
                        ...
                        else if (currInventory.StockAvailable.Value == 0 || currInventory.StockAvailable.Value < itemQty || currInventory.StockOnHand < itemQty)
                        {
                            Debug.WriteLine("Current Inventory Location/Qty key showed out of stock or not enough stock " +
                                            PickupLocationCode + " " + product.ProductCode + 
                                            " stock available: " + currInventory.StockAvailable +
                                            " purchase requested: " + itemQty);
                            var inventory =
                    ...
                    && directShipMethod)
                {
                    ...
                    Assert.AreNotEqual(productDirectShip.IsActive, 0,
                                        "Product: " + productToUse + " is visible in ProductAdmin and not Storefront.");
                    if (!productDirectShip.InventoryInfo.ManageStock.Value)
                    {
                        Debug.WriteLine("Product " + productDirectShip.ProductCode + " does not need inventory to purchase.");
                    }
                    else
                    {
                        var currInventory = LocationInventoryFactory.GetLocationInventory(
            ...
            if (submitOrder)
            {
                ...
                if (fullfillOrder)
                {
                    ...
                    {
                        // delete [a] reservation.  This is to make sure orders still fulfill after an inventory override.
                        var productReservations =
                            ProductReservationFactory.GetProductReservations(handler: ApiMsgHandler, filter: "OrderId eq " + getOrder.Id);

                        ProductReservationFactory.DeleteProductReservation(handler: ApiMsgHandler, productReservationId: (int)productReservations.Items[0].Id);
                    ...
                    {
                        // delete [a] reservation.  This is to make sure orders still fulfill after an inventory override.
                        var productReservations = ProductReservationFactory.GetProductReservations(handler: ApiMsgHandler, filter: "OrderId eq " + getOrder.Id);
        ...
        public Mozu.Api.Contracts.ProductRuntime.Product GetStoreFrontProduct(ServiceClientMessageHandler handler, string productCode)
        {
            ...
            {
                Debug.WriteLine("Error getting Product: " + productCode + " error message: " + ex.Message);
                return null;
            }
        }
        #endregion
